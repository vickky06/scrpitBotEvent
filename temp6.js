(function() {
    ...
    window.addEventListener('message', function(event1) {
        try {
            ...
            console.log('EVENT RECEIVED NEW' , event);
            console.log(event.event_code, "EVENT CODE");
            console.log(event.data.code, "DATA CODE");

            if (("custom-event" == event.event_code && "cb" === event.data.code)||"ym-bot-opened"==event.event_code) {
                console.log('value triggered for custom-event',event.event_code);
                // document.getElementsByClassName("title-parent")[0].remove()
                var a = document.getElementsByClassName("title")[0].remove();   
                // console.log('title execution',a);
                // while (a.length > 0) {
                //     a[0].parentNode.removeChild(a[0]);
                // }
                var b = document.getElementsByClassName("sub-title").remove();
